...
| 算法 | 特点 |
|------|------|
| **MD5** | 固定32位，不可逆，但可暴力破解，不推荐单独用 |
| **BCrypt** | 每次加密结果不同（加随机盐），安全，Spring Security 默认推荐 |

**原则**：密码不能明文存数据库，必须加密后存储。

### 2. Session 机制

- 登录成功后，服务器生成 `sessionId` 存浏览器 Cookie
- 之后浏览器每次请求自动带 sessionId
- 服务器根据 sessionId 找到用户会话，知道是谁

**比喻**：银行取号，后面凭小票办事。

### 3. Cookie

浏览器存的小段数据，由服务器设置，每次请求自动带回去。常用来存 sessionId。

### 4. Token

登录成功后服务器发一个加密字符串给前端，前端每次请求带上 token。服务器校验 token 有效性。比 session 更常用，适合前后端分离项目。

**和 AI 的 token 关系**：概念一样，都是身份凭证字符串，只是应用场景不同。
...
| 操作 | 公式 |
|------|------|
| 入队 | `rear = (rear + 1) % data.length`，`size++` |
| 出队 | `front = (front + 1) % data.length`，`size--` |
| 取队尾 | `data[(rear - 1 + data.length) % data.length]` |
| 判空 | `size == 0` |
| 判满 | `size == data.length` |

**`rear=0` 时队尾在最后一个位置**：因为 rear 指向下一个插入位置，rear=0 说明绕回开头，队尾是末尾。
...
```java
queue.offer(t);                        // 加入新请求
while (queue.peek() < t - 3000) {      // 移除超过 3000ms 的
    queue.poll();
}
return queue.size();                    // 剩余的都是最近的
```
...
| 分类 | 内容 |
|------|------|
| 概念 | Session、Cookie、Token、MD5、BCrypt |
...
|------|------|------|
| GET | 无 | `/api/books` |
| POST | Body → raw → JSON | `/api/books` |
| PUT | Body → raw → JSON | `/api/books/5` |
| DELETE | 无 | `/api/books/5` |

### 3. MyBatis 驼峰映射

**问题**：数据库列 `is_borrowed`（下划线）无法映射到 Java 属性 `isBorrowed`（驼峰），查询结果一直是默认值。

**解决**：`application.properties` 加：

```properties
mybatis.configuration.map-underscore-to-camel-case=true
```

**映射**：数据库列名和 Java 属性名的对应关系。加了配置后 `is_borrowed` 自动转 `isBorrowed`。

### 4. 借书失败排查

- SQL 正确、代码正确、数据库列正确
- 结果是**书已借出**，再借同一本会失败（符合业务逻辑）
- 不是 bug，是防重复借出的正常行为
...
`start = i - (len - 1) / 2`：由中心位置和长度反推起点。

### 424 替换后的最长重复字符

**方法**：滑动窗口 + 计数

- 窗口内需要替换的字符数 = 窗口长度 - 出现最多的字符数
- 需要替换数 ≤ k 窗口合法，> k 缩小左边界
...
| 模块 | 接口 |
|------|------|
| 图书 | GET/POST/PUT/DELETE + 分页 + 条件查询 + 借还书 |
| 用户 | 注册、登录、CRUD |

**注意**：`user` 表名是关键字，SQL 里加反引号。注册时用户名已存在会报 500 唯一约束冲突，换用户名即可。
...
**方法**：前缀和 + HashMap，0 当 -1

- 问题转化为找和为 0 的最长子数组
- map 存**第一次出现的位置**（和昨天 560 不同，昨天存次数）
...
- 拦截器会拦截未带 token 的请求，返回 401
- 测试参数校验需在 Headers 加 `token: 123`
- 校验失败返回 `{"code":500,"msg":"书名不能为空"}`
...
- `dp`：以当前元素结尾的最大子数组和
- `max`：遍历过程中的全局最大值
- 为什么返回 max 不返回 dp：dp 只是中间状态，max 才是最终答案

### 152 乘积最大子数组

**方法**：动态规划 + 维护最大最小值

- 负数乘负数变正数，所以遇到负数要交换 max 和 min
...
1. **定义状态**：`dp[i]` 表示什么
2. **写转移方程**：`dp[i]` 怎么由前几个推出
3. **更新答案**：每轮用 max/min 记录最终结果
...
| 检查项 | 内容 |
|------|------|
| 接口测试 | 图书 10 个 + 用户 6 个，全部正常 |
| 拦截器 | 图书带 token 通过，不带 401 |
| 参数校验 | 空字段返回友好提示 |
| 全局异常 | 统一返回格式 |
| README | 接口文档完整 |
| GitHub | 代码已 push |
...
- 图书接口要在 Headers 加 `token: 123`
- 用户接口不需要 token
- 测试时方法要选对（POST/PUT/DELETE）
- URL 要写对，别把参数和路径搞混
...
终止条件：`target == 0` 加入结果，`target < 0` 直接返回。
...
```java
String letters = map[digits.charAt(index) - '0'];  // 字符转索引
for (char c : letters.toCharArray()) {
    sb.append(c);
    backtrack(digits, index + 1, sb, result, map);
    sb.deleteCharAt(sb.length() - 1);  // 撤销最后加的字母
}
```
...
| 题 | start 变化 | 终止条件 | 特点 |
|------|------|------|------|
| 全排列 | 无 start，用 contains | path 长度够 | 顺序不同 |
| 子集 | `i+1` | 任意状态都加入 | 选或不选 |
| 组合总和 | `i`（可重复） | target==0 | 可重复选 |
| 电话号码 | `index+1` | 走完所有数字 | 每层选一个字母 |
